...
void loop() {
  ...
  //while (digitalRead(switch_on) == HIGH) {
...
//      Serial.println(casetemp);
    ...
    while (temp_good && speed_good) {           // You are under 60° C, and moving at least at 10km/h
      casetemp = 2;
     // if (millis() - timer_five_sec >= 3500) {  // Peltier on for more than 5 seconds ? 
...
//      Serial.println(casetemp);
    ...
    while (!temp_good && speed_good) { 
      ...
      speed_good = speed_KMpH >= min_speed;    // Still moving ? 
//     timer_five_sec = millis();              // Restart five seconds each time to make sure it is "zero" when exits CASE 4
//      Serial.print("CaseTemp =");
//      Serial.println(casetemp);
      //Serial.println("3");
    }
