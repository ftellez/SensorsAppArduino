...
void setup() {
 ...
 // while (!Serial);    // Do not start until serial monitor is open 
  ...
  if (datalogFile) {
    ...
    datalogFile.println("Speed / Temp-body / Temp-hotplate / MOSFET /Time");
    datalogFile.close();
    Serial.println("Test");
    Serial.println("Speed / T-body / T-hotplate / Mosfet / Time / Mosfet");
...
void logData() {
  ...
  if (millis() - timer_two_sec > 1000) { // approximately every 2 seconds print out the current stats
    ...
    if(LastTime <= CurrentTime) { // If stuff was typed in the serial monitor
      ...
      datalog += (String)segundosEjec + "," + duty_cycle + "," + casetemp; 
